...
void VelocitySensor_Cleanup(VelocitySensor *const me) { cleanUpRelations(me); }
...
void VelocitySensor_setItsMutex(VelocitySensor *const me,
                                struct Mutex *p_Mutex) {
  me->itsMutex = p_Mutex;
}

struct ResourceList *
VelocitySensor_getItsResourceList(const VelocitySensor *const me) {

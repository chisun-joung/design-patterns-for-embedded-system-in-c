...
void SensorMaster_Cleanup(SensorMaster *const me) { cleanUpRelations(me); }
...
void SensorMaster_gps_configure(SensorMaster *const me, short reqSatellites,
                                int useFast) {
  SimMutex_lock(me->itsSimMutex);
...
void SensorMaster_optical_configure(SensorMaster *const me, int wheelSize,
                                    int sensitivity) {
  SimMutex_lock(me->itsSimMutex);
  OpticalSpeedSensor_configure(me->itsOpticalSpeedSensor, wheelSize,
                               sensitivity);
  SimMutex_release(me->itsSimMutex);
}
...
void SensorMaster_setItsDopplerSpeedSensor(
    SensorMaster *const me, struct DopplerSpeedSensor *p_DopplerSpeedSensor) {
...
void SensorMaster_setItsGPSPositionSensor(
    SensorMaster *const me, struct GPSPositionSensor *p_GPSPositionSensor) {
...
void SensorMaster_setItsOpticalSpeedSensor(
    SensorMaster *const me, struct OpticalSpeedSensor *p_OpticalSpeedSensor) {
...
void SensorMaster_setItsSimMutex(SensorMaster *const me,
                                 struct SimMutex *p_SimMutex) {

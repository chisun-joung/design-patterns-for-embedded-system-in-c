...
void PatientData_Init(PatientData *const me) {
  ...
  me->crc =
      computeCRC((unsigned char *)&me->pData, sizeof(me->pData), 0xffff, 0);
}

void PatientData_Cleanup(PatientData *const me) { cleanUpRelations(me); }
...
struct AlarmManager *
PatientData_getItsAlarmManager(const PatientData *const me) {
  return (struct AlarmManager *)me->itsAlarmManager;
}

void PatientData_setItsAlarmManager(PatientData *const me,

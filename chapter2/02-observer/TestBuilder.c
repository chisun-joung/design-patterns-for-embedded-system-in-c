...
void TestBuilder_Init(TestBuilder *const me) { initRelations(me); }

void TestBuilder_Cleanup(TestBuilder *const me) { cleanUpRelations(me); }

// struct ArrhythmiaDetector* TestBuilder_getItsArrhythmiaDetector(const
// TestBuilder* const me) {
...
TestBuilder_getItsHistogramDisplay(const TestBuilder *const me) {
  return (struct HistogramDisplay *)&(me->itsHistogramDisplay);
}

// struct QRSDetector* TestBuilder_getItsQRSDetector(const TestBuilder* const
// me) {
...
// struct WaveformDisplay* TestBuilder_getItsWaveformDisplay(const TestBuilder*
// const me) {
...
static void initRelations(TestBuilder *const me) {
  ...
  // &(me->itsTMDQueue));
  // ArrhythmiaDetector_setItsTMDQueue(&(me->itsArrhythmiaDetector),
  // &(me->itsTMDQueue));
}

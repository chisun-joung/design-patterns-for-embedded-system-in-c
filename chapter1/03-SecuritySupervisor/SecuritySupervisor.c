...
void displayMsg(char *msg) { printf("%s\n", msg); }

int isCancel(char key) { return key == ESC_KEY; }

int isDigit(char key) { return ((key) >= '0' && (key) <= '9'); }

int isEnter(char key) { return key == ENTER_KEY; }

int isReset(char key) { return key == RESET_KEY; }
...
int isValid(char *pin) { return strcmp(pin, "1234"); }

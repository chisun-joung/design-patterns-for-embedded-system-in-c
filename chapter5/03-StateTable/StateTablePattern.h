...
/* This action pointer accepts the me object pointer and the passed character.
 * This means that the client must always send the character data with the
 * event.  */
...
/* This function returns a int that is treated as a boolean (0 == FALSE) and, if
 * present, is used to determine whether or not the transition is taken. Note
 * that this function takes no paramaeters (other than the object me pointer).
 */

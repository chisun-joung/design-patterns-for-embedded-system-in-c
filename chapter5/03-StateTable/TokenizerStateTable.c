...
void TokenizerStateTable_Init(TokenizerStateTable *const me) {
  ...
  te.entryActionPtr->aPtr.a0 =
      (ActionPtr0)TokenizerStateTable_enter_ProcessingWholePart;
  ...
  te.entryActionPtr->nParams = 0;
  te.entryActionPtr->aPtr.a0 =
      (ActionPtr0)TokenizerStateTable_enter_ProcessingFractionalPart;
  ...
  te.exitActionPtr = (ActionPtr)malloc(sizeof(ActionPtr));
  te.exitActionPtr->nParams = 0;
  te.exitActionPtr->aPtr.a0 =
      (ActionPtr0)TokenizerStateTable_exit_ProcessingWholePart;
  ...
  te.exitActionPtr->nParams = 0;
  te.exitActionPtr->aPtr.a0 =
      (ActionPtr0)TokenizerStateTable_exit_ProcessingFractionalPart;
  te.transActionPtr = (ActionPtr)malloc(sizeof(ActionPtr));
  te.transActionPtr->nParams = 1;
...
void TokenizerStateTable_eventDispatch(TokenizerStateTable *const me, Event e) {
  ...
  if (me->stateID >= NULL_STATE &&
      me->stateID <= GN_PROCESSINGFRACTIONALPART_STATE) {
        ...
        if (takeTransition) {
          if (me->table[me->stateID][e.eType].exitActionPtr != NULL)
            if (me->table[me->stateID][e.eType].exitActionPtr->nParams == 0)
              ...
              me->table[me->stateID][e.eType].exitActionPtr->aPtr.a1(me,
                                                                     e.ed.c);
          if (me->table[me->stateID][e.eType].transActionPtr != NULL)
            if (me->table[me->stateID][e.eType].transActionPtr->nParams == 0)
              ...
              me->table[me->stateID][e.eType].transActionPtr->aPtr.a1(me,
                                                                      e.ed.c);
          if (me->table[me->stateID][e.eType].entryActionPtr != NULL)
            if (me->table[me->stateID][e.eType].entryActionPtr->nParams == 0)
              ...
              me->table[me->stateID][e.eType].entryActionPtr->aPtr.a1(me,
                                                                      e.ed.c);
...
void TokenizerStateTable_enter_ProcessingFractionalPart(
    TokenizerStateTable *const me) {}

void TokenizerStateTable_enter_ProcessingWholePart(
    TokenizerStateTable *const me) {
...
void TokenizerStateTable_exit_ProcessingFractionalPart(
    TokenizerStateTable *const me) {}

void TokenizerStateTable_exit_ProcessingWholePart(
    TokenizerStateTable *const me) {}

struct Mutex *
TokenizerStateTable_getItsMutex(const TokenizerStateTable *const me) {
  return (struct Mutex *)me->itsMutex;
}

void TokenizerStateTable_setItsMutex(TokenizerStateTable *const me,
                                     struct Mutex *p_Mutex) {
  me->itsMutex = p_Mutex;
}

TokenizerStateTable *TokenizerStateTable_Create(void) {
  TokenizerStateTable *me =
      (TokenizerStateTable *)malloc(sizeof(TokenizerStateTable));
  if (me != NULL)

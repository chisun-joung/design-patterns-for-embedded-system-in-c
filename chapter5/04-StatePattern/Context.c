...
void Context_evDigit(Context *const me, char c) {
  ...
  if (me->stateList[me->currentState]->aSet.evDigitHandler(
          me->stateList[me->currentState], c))
    me->stateList[me->currentState]->aSet.entryAction(
...
void Context_evDot(Context *const me) {
  ...
  /* this call does the entry and transition actions */
  /* and updates the currentState variable */
  if (me->stateList[me->currentState]->aSet.evDotHandler(
          me->stateList[me->currentState]))
    me->stateList[me->currentState]->aSet.entryAction(
        me->stateList[me->currentState]);
  Mutex_release(me->itsMutex);
...
void Context_evEndOfString(Context *const me) {
  ...
  /* this call does the entry and transition actions */
  /* and updates the currentState variable */
  if (me->stateList[me->currentState]->aSet.evEOSHandler(
          me->stateList[me->currentState]))
    me->stateList[me->currentState]->aSet.entryAction(
        me->stateList[me->currentState]);
  Mutex_release(me->itsMutex);
...
void Context_evWhiteSpace(Context *const me) {
  ...
  /* this call does the entry and transition actions */
  /* and updates the currentState variable */
  if (me->stateList[me->currentState]->aSet.evWSHandler(
          me->stateList[me->currentState]))
    me->stateList[me->currentState]->aSet.entryAction(
        me->stateList[me->currentState]);
  Mutex_release(me->itsMutex);

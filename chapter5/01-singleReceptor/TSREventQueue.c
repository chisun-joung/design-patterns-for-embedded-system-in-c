...
void TSREventQueue_Cleanup(TSREventQueue *const me) { cleanUpRelations(me); }

void TSREventQueue_isEmpty(TSREventQueue *const me) { return me->size <= 0; }
...
Event TSREventQueue_pull(TSREventQueue *const me) {
  ...
  if (!TSREventQueue_isEmpty(me)) {
    e = me->q[me->tail];
    me->tail = (me->tail + 1) % QSIZE me->size -= 1;
  };
  Mutex_release(me->itsMutex);
  return e;
}

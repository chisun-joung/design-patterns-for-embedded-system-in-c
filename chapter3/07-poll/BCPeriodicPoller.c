...
struct BCTimer *
BCPeriodicPoller_getItsBCTimer(const BCPeriodicPoller *const me) {
...
int BCPeriodicPoller_getItsBreathingCircuitSensor(
...
void BCPeriodicPoller_clearItsBreathingCircuitSensor(
    BCPeriodicPoller *const me) {
  {
    int pos;
    for (pos = 0; pos < 3; ++pos) {
...
BCPeriodicPoller_getItsMedicalDisplay(const BCPeriodicPoller *const me) {
  return (struct MedicalDisplay *)me->itsMedicalDisplay;
}

void BCPeriodicPoller_setItsMedicalDisplay(
...
void BCPeriodicPoller_setItsBCTimer2(BCPeriodicPoller *const me,
                                     struct BCTimer *p_BCTimer) {
  me->itsBCTimer = p_BCTimer;
}

void BCPeriodicPoller_setItsBCTimer1(BCPeriodicPoller *const me,
                                     struct BCTimer *p_BCTimer) {
  if (me->itsBCTimer != NULL) {
    BCTimer_setItsBCPeriodicPoller(me->itsBCTimer, NULL);
  }

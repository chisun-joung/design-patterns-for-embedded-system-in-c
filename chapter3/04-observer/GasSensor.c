...
void GasSensor_Cleanup(GasSensor *const me) { cleanUpRelations(me); }
  ...
  for (pos = 0; pos < 100; pos++)
    if (me->itsGasNH[pos])
      if (me->itsGasNH[pos]->acceptorPtr) {
        printf("Client %d: InstancePtr=%p, acceptPtr=%p/n", pos,
               me->itsGasNH[pos]->instancePtr, me->itsGasNH[pos]->acceptorPtr);
      };
}
...
void GasSensor_unsubscribe(GasSensor *const me,
                           const gasDataAcceptorPtr *aPtr) {
  int pos;
  for (pos = 0; pos < 100; ++pos) {
    if (me->itsGasNH[pos])
...
void GasSensor_addItsGasNH(
    GasSensor *const me,
    struct GasNotificationHandle *p_GasNotificationHandle) {
  int pos;
  for (pos = 0; pos < 100; ++pos) {
    if (!me->itsGasNH[pos]) {

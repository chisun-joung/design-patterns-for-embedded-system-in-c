...
void ButtonDriver_Cleanup(ButtonDriver *const me) { cleanUpRelations(me); }
...
void ButtonDriver_setItsButton(ButtonDriver *const me,
                               struct Button *p_Button) {
  if (p_Button != NULL) {
...
void ButtonDriver_setItsMicrowaveEmitter(
    ButtonDriver *const me, struct MicrowaveEmitter *p_MicrowaveEmitter) {
...
static void cleanUpRelations(ButtonDriver *const me) {
  if (me->itsButton != NULL) {
    struct ButtonDriver *p_ButtonDriver =
        Button_getItsButtonDriver(me->itsButton);
...
void ButtonDriver_setItsButton2(ButtonDriver *const me,
                                struct Button *p_Button) {
  me->itsButton = p_Button;
}

void ButtonDriver_setItsButton1(ButtonDriver *const me,
                                struct Button *p_Button) {
  if (me->itsButton != NULL) {
    Button_setItsButtonDriver(me->itsButton, NULL);
  }
